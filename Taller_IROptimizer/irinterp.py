...
class IRInterpreter:
	...
	def _exec_global(self, inst: tuple):
		...
		# Formato de strings: ('.STR0', [bytes]) o similar
		if op.startswith("."):
			_, bytes_list = inst
			self.data[op] = [int(b) for b in bytes_list]
			return
...
if __name__ == "__main__":
	...
	if sys.platform != 'ios':
		if len(sys.argv) != 3:
			raise SystemExit("Usage: python irinterp.py <filename> <optimizacion>")
		filename = sys.argv[1]
		optimizacion = sys.argv[2] if len(sys.argv) > 2 else None
	else:
	...
	if filename:
		...
		irOptimizado = IROptimizer.optimize(IRCodeGen.generate(ast), level=int(optimizacion))

		interp = IRInterpreter(irOptimizado, trace=True)
		interp.run("main")